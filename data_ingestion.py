...
class MinimalSignalGenerator:
    ...
    def __init__(self, data_ingestion: PolygonDataIngestion):
...
def setup_data_ingestion(use_test_data: bool = False, base_prices: Optional[Dict[str, float]] = None):
    """
    Setup function to initialize data ingestion
    
    Args:
        use_test_data: If True, use TestDataIngestion instead of real Polygon data
        base_prices: Base prices for test data (only used when use_test_data=True)
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "demo":
        ...
        # Quick test of both data ingestors
        test_ingestion = TestDataIngestion()
        ...
        try:
            ...
            try:
                data_ingestion = setup_data_ingestion(use_test_data=False)
                print("Using real Polygon.io data")
            except ValueError as e:
                print(f"Polygon API key not found: {e}")
                print("Falling back to test data...")
                data_ingestion = setup_data_ingestion(use_test_data=True)
