...
Examples:
  ...
  stratequeue deploy --strategy sma.py --symbol AAPL,MSFT --data-source demo
  
  # Multi-strategy mode (comma-separated values)
  stratequeue deploy --strategy sma.py,momentum.py,random.py --allocation 0.4,0.35,0.25 --symbol AAPL,MSFT --data-source demo
  
  # Multi-strategy with 1:1 strategy-symbol mapping
  stratequeue deploy --strategy sma.py,random.py --allocation 0.5,0.5 --symbol ETH,AAPL --data-source demo
  
  # Multi-strategy with custom strategy IDs
  stratequeue deploy --strategy sma.py,momentum.py --strategy-id sma_cross,momentum_trend --allocation 0.6,0.4 --symbol AAPL
  
  # Multi-strategy with dollar allocations
  stratequeue deploy --strategy sma.py,momentum.py --allocation 1000,500 --symbol AAPL --broker alpaca --paper
  
  # Multi-strategy with different granularities per strategy
  stratequeue deploy --strategy sma.py,momentum.py --allocation 0.6,0.4 --granularity 1m,5m --symbol ETH,BTC
  
  # Multi-strategy with different data sources per strategy  
  stratequeue deploy --strategy sma.py,momentum.py --allocation 0.6,0.4 --data-source polygon,coinmarketcap --symbol AAPL,ETH
  
  # Single value applies to all strategies
  stratequeue deploy --strategy sma.py,momentum.py --allocation 0.5,0.5 --granularity 1m --broker alpaca --symbol ETH
  
  # Run with real Polygon data
  stratequeue deploy --strategy sma.py --symbol AAPL --data-source polygon --lookback 50
  
  # Paper trading (default behavior)
  stratequeue deploy --strategy sma.py --symbol AAPL --paper
  
  # Live trading (use with caution!)
  stratequeue deploy --strategy sma.py --symbol AAPL --live
  
  # Disable trading execution (signals only)
  stratequeue deploy --strategy sma.py --symbol AAPL --no-trading
  
  # Multi-strategy with live trading
  stratequeue deploy --strategy sma.py,momentum.py --allocation 0.6,0.4 --symbol AAPL,MSFT --live
