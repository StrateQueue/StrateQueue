...
class DaemonCommand(BaseCommand):
    ...
    def _call_daemon_api(
        ...
        timeout: int = DAEMON_REQUEST_TIMEOUT,
        ...
        try:
            ...
            return False, {
                "error": "Daemon control requires 'requests' package. Install with: pip3.11 install requests"
            }
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        subparsers = parser.add_subparsers(
            dest="daemon_action", help="Daemon actions", metavar="action"
        )
        ...
        # Stop daemon
        subparsers.add_parser("stop", help="Stop the daemon process")
        ...
        strategy_parser = subparsers.add_parser(
            "strategy", help="Control strategies in the running daemon"
        )
        strategy_subparsers = strategy_parser.add_subparsers(
            dest="strategy_action", help="Strategy actions", metavar="strategy_action"
        )

        # Deploy strategy
        deploy_strategy_parser = strategy_subparsers.add_parser(
            "deploy", help="Deploy a new strategy to the running daemon"
        )
        deploy_strategy_parser.add_argument(
            "--strategy", required=True, help="Path to strategy file"
        )
        deploy_strategy_parser.add_argument("--symbol", required=True, help="Trading symbol")
        deploy_strategy_parser.add_argument("--strategy-id", help="Custom strategy identifier")
        deploy_strategy_parser.add_argument(
            "--allocation", type=float, default=0.1, help="Allocation percentage (default: 0.1)"
        )

        # Pause strategy
        pause_strategy_parser = strategy_subparsers.add_parser(
            "pause", help="Pause a running strategy"
        )
        pause_strategy_parser.add_argument("strategy_id", help="Strategy identifier to pause")

        # Resume strategy
        resume_strategy_parser = strategy_subparsers.add_parser(
            "resume", help="Resume a paused strategy"
        )
        resume_strategy_parser.add_argument("strategy_id", help="Strategy identifier to resume")

        # Undeploy strategy
        undeploy_strategy_parser = strategy_subparsers.add_parser(
            "undeploy", help="Remove a strategy from the daemon"
        )
        undeploy_strategy_parser.add_argument("strategy_id", help="Strategy identifier to undeploy")

        # Rebalance portfolio
        rebalance_parser = strategy_subparsers.add_parser(
            "rebalance", help="Rebalance strategy allocations"
        )
    ...
    def _deploy_strategy(self, args: Namespace) -> int:
        ...
        payload = {"strategy": strategy_path, "symbol": symbol, "allocation": allocation}
        ...
        success, response = self._call_daemon_api(
            "/strategy/deploy", method="POST", payload=payload
        )
        if success:
    ...
    def _pause_strategy(self, args: Namespace) -> int:
        ...
        success, response = self._call_daemon_api(
            "/strategy/pause", method="POST", payload={"strategy_id": strategy_id}, timeout=10
        )
        if success:
    ...
    def _resume_strategy(self, args: Namespace) -> int:
        ...
        success, response = self._call_daemon_api(
            "/strategy/resume", method="POST", payload={"strategy_id": strategy_id}, timeout=10
        )
        if success:
    ...
    def _undeploy_strategy(self, args: Namespace) -> int:
        ...
        success, response = self._call_daemon_api(
            "/strategy/undeploy", method="POST", payload={"strategy_id": strategy_id}, timeout=10
        )
        if success:
    ...
    def _rebalance_portfolio(self, args: Namespace) -> int:
        ...
        success, response = self._call_daemon_api(
            "/portfolio/rebalance", method="POST", payload={"allocations": allocations_dict}
        )
        if success:
