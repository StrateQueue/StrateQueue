...
class TradingDaemon:
    ...
    def __init__(self):
        self.system: Optional[LiveTradingSystem] = None
    ...
    async def _create_trading_system(self, cli_args: Dict[str, Any]) -> None:
        ...
        try:
            ...
            broker_type = cli_args.get("broker", None)
            duration_minutes = cli_args.get("duration", 60)
            
            # Always use multi-strategy mode for runtime flexibility
            strategies = cli_args.get("_strategies", [cli_args.get("strategy")])
            is_multi_strategy = True  # Force multi-strategy mode for hot-swap capabilities
    ...
    def _create_multi_strategy_config(self, cli_args: Dict[str, Any]) -> Optional[str]:
...
def run_daemon(bind: str = DEFAULT_DAEMON_HOST, port: int = DEFAULT_DAEMON_PORT, log_file: Optional[str] = None):
    """
